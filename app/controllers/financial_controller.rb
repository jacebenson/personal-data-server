class FinancialController < ApplicationController
  ...
  def view_investments
    ...
    if params[:search].present?
      @investments = @investments.where("description ILIKE ? OR security ILIKE ?", 
  ...
  def detect_fidelity_file_type(file)
    ...
    # Look for headers that indicate transaction vs position files
    if content.include?("Account") && content.include?("Symbol") && content.include?("Quantity")
      "positions"
    else
      "transactions"
    end
