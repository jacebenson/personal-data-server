...
class Api::V1::Mcp::SearchController < Api::V1::Mcp::BaseController
  ...
  def search_all_data
    ...
    data_types = @sanitized_params[:data_types] || %w[financial health calendar entertainment]
  ...
  def search_financial(query, timeframe, limit)
    ...
    if defined?(BankStatement) && current_user.respond_to?(:bank_statements)
      transaction_scope = current_user.bank_statements.where("description LIKE ? COLLATE NOCASE OR category LIKE ? COLLATE NOCASE",
                                                           "%#{query}%", "%#{query}%")
    ...
    if defined?(AmazonOrder)
      order_scope = current_user.amazon_orders.where("title LIKE ? COLLATE NOCASE OR category LIKE ? COLLATE NOCASE",
                                                    "%#{query}%", "%#{query}%")
  ...
  def search_health(query, timeframe, limit)
    ...
    model_associations.each do |model_name, association_name|
      ...
      if text_columns.any?
        where_clause = text_columns.map { |col| "#{col} LIKE ? COLLATE NOCASE" }.join(" OR ")
  ...
  def search_calendar(query, timeframe, limit)
    ...
    if defined?(CalendarEvent)
      event_scope = current_user.calendar_events.where("title LIKE ? COLLATE NOCASE OR description LIKE ? COLLATE NOCASE OR location LIKE ? COLLATE NOCASE",
                                                      "%#{query}%", "%#{query}%", "%#{query}%")
  ...
  def search_entertainment(query, timeframe, limit)
    ...
    if defined?(NetflixViewingActivity)
      netflix_scope = current_user.netflix_viewing_activities.where("title LIKE ? COLLATE NOCASE", "%#{query}%")
    ...
    if defined?(YoutubeWatchHistory)
      youtube_scope = current_user.youtube_watch_histories.where("title LIKE ? COLLATE NOCASE OR channel LIKE ? COLLATE NOCASE",
                                                                "%#{query}%", "%#{query}%")
    ...
    if defined?(AudibleLibraryItem)
      audible_scope = current_user.audible_library_items.where("title LIKE ? COLLATE NOCASE OR author LIKE ? COLLATE NOCASE",
                                                             "%#{query}%", "%#{query}%")
