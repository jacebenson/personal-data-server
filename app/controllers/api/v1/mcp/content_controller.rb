    ...
    if defined?(AudibleLibraryItem)
      ...
      if mood.present?
        ...
        when 'learning', 'educational'
          audible_books = audible_books.where("title ILIKE ? OR title ILIKE ? OR title ILIKE ?", 
                                             "%how to%", "%guide%", "%master%")
        when 'relaxing', 'fiction'
          audible_books = audible_books.where("title ILIKE ? OR title ILIKE ?", "%novel%", "%story%")
        when 'motivational', 'self-help'
          audible_books = audible_books.where("title ILIKE ? OR title ILIKE ?", "%success%", "%life%")
        end
      end
